...
# [Median Splits (1600)](https://codeforces.com/problemset/problem/2103/C)
## 关于中位数的重要性质：
对于任何一个数组，要让他的中位数med<=k,等价于：
子数组中<=k的数的个数>=大于k的数的个数。
可以想象数组有两个阵营，分别是<=k阵营和>k阵营，他们中间有一条分界线，小数阵营比大叔阵营大说明分界线往右偏移，那么中位数肯定就小于k。阵营相等的情况特判一下就好了，比如有四个数两两分组，那第二个数是中位数，它属于小数阵营。
## 关于对于无序数组怎么判断一段区间的小数多还是大数多：
很简单的问题，用一个新数组表示每个位置的情况，比如小数就是1，大数就是-1，然后算这个数组的前缀和就好了。因为一段区间的和加起来的值如果>=0就代表小数比大数多，反之亦然
